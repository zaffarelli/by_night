...
*/
    ...
    gaugeStat(name,value,ox,oy,source,withTemp=false,automax=false){
        let me = this;
        let type=name;
        let item = source.append('g');
        let lines = 1;
        ...
        dot.append('circle')
            .attr('cx',function(d){
                let cx =  ox+(d-4.5)*((me.dot_radius+1)*2);
                if (d>=10){
    ...
    fillAdvantages(basey){
        ...
        if (me.data['creature']=='garou'){
            oy += 1.5*me.stepy;
            me.gaugeStat('Rank',me.data['rank'],ox+me.stepx*19,oy,me.character);
        }else if (me.data['creature']=='kindred'){
